...
В конце прошлого года мне потребовался undo/redo инструмент для Java-проекта, который, помимо стандартных для этого концепта задач, умел бы сохранять историю команд (это с прицелом на мой проект для Android и его регулярное пересоздание вьюшек). Поискал, не нашел, взялся.

Результатом стала библиотечка JUndo. 

Ее дополнительные возможности:

- Сохранение истории команд для использование где-то в другое время, в другом месте
- Поддержание механизма версионности субъектов для поддержания миграции на стороне восстановления истории
...
Честно говоря, сделать слишком просто не получилось, в-основном, из-за поддержания концепции переноса истории команд в другой адресный контекст. 
...
Те же, которые не имплементируют, требуется настраивать ручками при сохранении/восстановлении. Но, хоть это и звучит грустно, на самом деле не так уж и сложно.
...
Общим правилом является следующее: виджеты, вьюшки, ресурсы и все, что является частью локального контекста следует подключать в локальные контексты стека и использовать динамически.
Если сохранить их вместе со стеком команд, то при пересоздании в новом адресном пространстве обязательно случится неприятность.
...
- класс не имплементирует Serializable и даже не сериализуется в JSON без бубна, поэтому мы не будем использовать его в полях команд. Вместо этого мы будем сохранят его свойства, которыми конкретно управляем: ColorUndo будет хранить oldColor/new Color, RadiusUndo - oldRadius/newRadius и так далее
...
```java
UndoPacket undoManager = UndoPacket.deserialize(s, this);
```
